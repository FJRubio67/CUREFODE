...
km <- survfit(Surv(colonDC$FUyear,colonDC$status) ~ 1)

plot(km$time, km$surv, type = "l", col = "black", lwd = 2, lty = 1, 
     ylim = c(0,1), xlab = "Time", ylab = "Survival")


survtimes <- as.vector(sort(colonDC$FUyear))
status <- as.logical(colonDC$status)
...
plot(km$time, km$surv, type = "l", col = "black", lwd = 2, lty = 1, 
     ylim = c(0,1), xlab = "Time", ylab = "Survival")
curve(fspgw,0,25, add = T, lwd = 2, ylim = c(0,1),
      col = "blue")
